...
import EditCounsellorPopup from "../../../ui/counsellors/editpopup/editpopup"; // Add correct path
...
export default function CounsellorActivePage() {
  ...
  const handleEditCounsellor = (updated: Counsellor) => {
    setCounsellors(prev => prev.map(c => (c._id === updated._id ? updated : c)));
  ...
  const handleDeleteCounsellor = async (id: string) => {
    ...
    setDeletingId(id);
    try {
      const res = await fetch(`/admind/api/counsellors/active/${id}`, { method: "DELETE" });
      if (res.ok) {
  ...
  return (
    <div className="p-6 bg-[#E9F0F6] min-h-screen">
            ...
            {counsellors.map((user, i) => (
              <tr key={i} className="border-t border-gray-200 hover:bg-gray-50">
                ...
                <td className="px-6 py-4 text-center flex justify-center gap-3 text-purple-600">
                  {permissions?.counselor?.read !== false && (
                    <button
                  ...
                  {permissions?.counselor?.update && (
                    <button
                      onClick={() => {
                        setSelectedCounsellor(user);
      ...
      {showEditModal && selectedCounsellor && (
        <EditCounsellorPopup
          counsellor={selectedCounsellor}
          onClose={() => setShowEditModal(false)}
          onSave={(updatedCounsellor) => {
            setShowEditModal(false);
            handleEditCounsellor(updatedCounsellor);
          }}
        />
      )}
    </div>
