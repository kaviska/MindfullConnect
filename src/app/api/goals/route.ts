...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const goals = new Goals({ title: title.trim(), description: description.trim(), counsellor_id });
        await goals.save();
        return NextResponse.json({ message: "Goal created successfully", data: goals }, { status: 201 });
    } catch (error) {
...
export async function PUT(req: NextRequest) {
    ...
    try {
        ...
        const updatedGoal = await Goals.findByIdAndUpdate(
            id,
            { ...(title && { title: title.trim() }), ...(description && { description: description.trim() }) },
            { new: true }
        );
        ...
        return NextResponse.json({ message: "Goal updated successfully", data: updatedGoal }, { status: 200 });
    } catch (error) {
...
export async function DELETE(req: NextRequest) {
    ...
    try {
        ...
        return NextResponse.json({ message: "Goal deleted successfully", data: deletedGoal }, { status: 200 });
    } catch (error) {
