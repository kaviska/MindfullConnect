...
const Sidebar = ({ collapsed, setCollapsed, windowWidth }: SidebarProps) => {
    ...
    if (!isMobile) {
        return (
            <div className={`bg-white shadow-xl border-r border-gray-100 h-screen flex flex-col transition-all duration-300 ease-in-out ${
                collapsed ? 'w-20' : 'w-64'
            }`}>
                <SidebarContent />
            </div>
    ...
    return (
        <>
            ...
            {/* Mobile Sidebar */}
            <div className={`fixed left-0 top-0 h-full w-80 bg-white shadow-2xl transform transition-transform duration-300 ease-in-out z-50 md:hidden ${
